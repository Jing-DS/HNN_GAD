...
import torch
from layers import GraphConvolution, Centralization

import manifolds
from manifolds.utils import acosh
from layers import LorentzGraphConvolution, LorentzCentralization, HyperbolicGraphConvolution, HyperbolicCentralization
...
class Encoder(nn.Module):
    def __init__(self, dropout, act, use_bias, layer_dims=None):
        ...
        for i in range(len(layer_dims) - 1):
            ...
            gc_layers.append(GraphConvolution(in_dim, out_dim, dropout, act, use_bias))
...
class GNN_Euclidean_mp(nn.Module):
    def __init__(self, dropout, act, use_bias, layer_dims=None):
        super(GNN_Euclidean_mp, self).__init__()
        act = getattr(F, act)
...
class Encoder_LORENTZ(nn.Module):
    def __init__(self, manifold, use_bias, dropout, layer_dims=None):
        ...
        for i in range(len(layer_dims) - 1):
            in_dim, out_dim = layer_dims[i], layer_dims[i + 1]
            gc_layers.append(LorentzGraphConvolution(manifold, in_dim, out_dim, use_bias, dropout))
...
class GNN_LORENTZ_mp(nn.Module):
    def __init__(self, manifold, use_bias, dropout, layer_dims=None):
        super(GNN_LORENTZ_mp, self).__init__()
...
class Encoder_POINCARE(nn.Module):
    def __init__(self, manifold, c, dropout, act, use_bias, layer_dims=None):
        ...
        for i in range(len(layer_dims) - 1):
            in_dim, out_dim = layer_dims[i], layer_dims[i + 1]
            gc_layers.append(HyperbolicGraphConvolution(manifold, in_dim, out_dim, c, c, dropout, act, use_bias, False, False))
...
class GNN_POINCARE_mp(nn.Module):
    def __init__(self, manifold, c, dropout, act, use_bias, layer_dims=None):
        super(GNN_POINCARE_mp, self).__init__()
        self.manifold = getattr(manifolds, manifold)()
        self.c = c
        act = getattr(F, act)
...
#         x_hyp = self.manifold.proj(x_hyp, c=self.c)
        ...
        x_emb = self.shared_encoder(x_hyp, adj)
        x_hat = self.attr_decoder(x_emb, adj)
        struc_decode = self.struct_decoder(x_emb, adj)
        return x_hat, struc_decode
